...
if [[ "$INTERACTIVE" -eq 1 ]]; then
    ...
    declare -A VAR_DESC=(
        ...
        ["UPLOAD_RETENTION_DAYS"]="Retensi upload (hari) sebelum dihapus cron — default 90, 0 = nonaktif"
        ["NEON_STORAGE_QUOTA_MB"]="Kuota storage Neon (MB) untuk laporan pemakaian & alert upload"
        ["STORAGE_ALERT_THRESHOLD_PCT"]="Ambang alert storage (persen kuota; default 80, <= 0 nonaktif)"
        ["STORAGE_ALERT_HYSTERESIS_PCT"]="Selisih turun untuk me-reset alert storage (persen; default 10)"
    )
...
OPTIONAL_KEYS=("SMTP_HOST" "SMTP_PORT" "SMTP_USER" "SMTP_PASS" "SMTP_FROM" "ADMIN_EMAIL" "FONNTE_TOKEN" "ADMIN_PHONE" "TELEGRAM_BOT_TOKEN" "TELEGRAM_CHAT_ID" "SENTRY_DSN" "NEXT_PUBLIC_SENTRY_DSN" "SENTRY_AUTH_TOKEN" "CRON_SECRET" "NEON_API_KEY" "NEON_PROJECT_ID" "UPLOAD_RETENTION_DAYS" "NEON_STORAGE_QUOTA_MB" "STORAGE_ALERT_THRESHOLD_PCT" "STORAGE_ALERT_HYSTERESIS_PCT" "DATABASE_URL_DIRECT" "REDIS_URL" "LOKI_URL" "SITE_DOMAIN" "ALLOWED_ORIGINS" "APP_DEBUG")
