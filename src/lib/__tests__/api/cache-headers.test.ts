...
describe("Cache-Control headers on public GET routes", () => {
  ...
  const cases: [string, () => Promise<Response>, string][] = [
    ...
    [
      "/api/site-settings",
      // GET /api/site-settings kini dibungkus withErrorHandling, sehingga
      // fungsi terekspor bertipe (req, ...) — request mock wajib dikirim.
      // (Catatan lama tentang arity 0 sudah tidak berlaku sejak adopsi wrapper.)
      () =>
        siteSettingsGET(
          asNextRequest(
